...
for i in range(10):
    input2=train_set[i][0]
    fig, ax = plt.subplots()
    im = ax.imshow((input2.numpy())[0]*0.3081-0.1307, cmap='binary')
...
nn=Net()
#nn_state_dict = torch.load("tmp/model_simple.txt")
...
def train(model, optimizer, loss_fn, train_loader, val_loader, epochs=50):
    ...
    for epoch in range(1, epochs+1):
        ...
        torch.save(nn.state_dict(),'tmp/model_simple.txt')


train(nn,optimizer, F.cross_entropy, train_loader, val_loader,epochs=15)
